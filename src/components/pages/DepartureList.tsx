...
export default function DepartureList() {
  ...
  // 초기 로딩 시 `location.state`를 저장하고, 없을 경우 `localStorage`에서 불러오기
  ...
  const handleSelectFlight = (flight: FlightData) => {
    ...
    const validReturnFlights = flightData.returnFlights || []

    // 출발편 선택 후 ArrivalList로 이동
    navigate('/arrival-list', {
      replace: true, // 뒤로 가기를 해도 state가 유지되도록 변경
      state: {
        flights: flightData.departureFlights, // 선택한 출발편
        returnFlights: validReturnFlights, // 도착편 리스트
        origin,
        destination,
        departureDate,
        returnDate,
            ...
            <Container>
              ...
              >
                ...
                >
                  ...
                  <Box paddingRight={8}>
                    <Typography variant="subtitle1">{departureDate}</Typography>
                  </Box>
                </Box>
                <Divider
                  orientation="vertical"
